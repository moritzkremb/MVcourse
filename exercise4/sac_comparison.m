...
pts = noisy_plane(plane_x, plane_y, plane_z, plane_offset, npts, x_spread, y_spread, z_spread, z_norm_spread, gauss_prop);

% show cloud and correct plane
figure(1); clf
hold on
plotPlanes(pts, plane_x, plane_y, plane_z, plane_offset)
plotPointCloud(pts)
...
plotPlanes(pts, plane_eqs(:, 1), plane_eqs(:, 2), plane_eqs(:, 3), plane_eqs(:, 4))
% actual plane
plotPlanes(pts, plane_x, plane_y, plane_z, plane_offset, [], 'r')


%% create parameter settings
params.confidence = 0.99;
params.inlier_margin = 0.5;
...
repetition_vals = [5, 10]; % change this
...
for cur_params = plist
    ...
    for n_reps = repetition_vals
        ...
        for i = 1:n_reps
            [a,b,c,d,inliers,sample_count] = fitPlane(pts, cur_params);
            plane_eqs(i, :) = [a b c d];
